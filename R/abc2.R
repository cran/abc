    ...
    if(rejmethod){
        ...
        if(method == "neuralnet"){
           ...
           #lambda <- sample(lambda, numnet, replace=T)
            ...
            #                 size = sizenet, trace = trace, linout = linout, maxit = maxit, ...)
            #    cat(i)
            	    fv[,parcur,i] <- drop(cbind(1,scaled.sumstat[wt1,])%*%(coef(fit1)[i,]))
                	pred[parcur,i] <- drop(c(1, target) %*%(coef(fit1)[i,]))
            					}
            }
           # cat("\n")
            pred.med <- apply(pred, 1, median)
            ...
            if(hcorr == TRUE){
                 ...
                 for(parcur in (1:numparam))
            	{
                	...
                	for (i in 1:numnet){
                    	#cat(i)
                    	fv2[,parcur,i] <- drop(cbind(1,scaled.sumstat[wt1,])%*%(coef(fit2)[i,]))
						pred2[parcur,i] <- drop(c(1, target) %*%(coef(fit2)[i,]))         
									}
                }
                cat("\n")
                pred.sd <- sqrt(exp(apply(pred2, 1, median)))
